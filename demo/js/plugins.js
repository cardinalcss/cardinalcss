...
0;i<css.length;i++)if(css[i])rules=rules.replace(foundProps[i],css[i]);var remcss=document.createElement("style");remcss.setAttribute("type","text/css");remcss.id="remReplace";document.getElementsByTagName("head")[0].appendChild(remcss);remcss.styleSheet.cssText=rules},xhr=function(url,callback,i){if(window.XDomainRequest){var xdr=new XDomainRequest;xdr.open("get",url);xdr.onload=function(){callback(xdr,i)};xdr.onerror=function(){alert("IE XDR load fail.")};xdr.send()}else{var xhr=getXMLHttpRequest();
xhr.open("GET",url,true);xhr.send();if(window.XMLHttpRequest)xhr.onreadystatechange=function(){if(xhr.readyState===4)callback(xhr,i);else;};else xhr.onreadystatechange=new function(){if(xhr.readyState===4)callback(xhr,i);else;}}},removeComments=function(css){var start=css.search(/\/\*/),end=css.search(/\*\//);if(start>-1&&end>start){css=css.substring(0,start)+css.substring(end+2);return removeComments(css)}else return css},getXMLHttpRequest=function(){if(window.XMLHttpRequest)return new XMLHttpRequest;
...
0)fontSize=body.currentStyle["fontSize"].replace("pt","");else fontSize=body.currentStyle["fontSize"].replace("%","")/100*16;else if(window.getComputedStyle)fontSize=document.defaultView.getComputedStyle(body,null).getPropertyValue("font-size").replace("px","");processSheets()}else;})(window);
