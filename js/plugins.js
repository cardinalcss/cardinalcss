...
sheets[i].href;xhr(links[i],matchcss,i)}},matchcss=function(response,i){var pattern=/[\w\d\s\-\/\\\[\]:,.'"*()<>+~%#^$_=|]+\{[\w\d\s\-\/\\%#:;,.'"*()]+\d*\.{0,1}\d+rem[\w\d\s\-\/\\%#:;,.'"*()]+\}/g,current=response.responseText.match(pattern),remPattern=/\d*\.{0,1}\d+rem/g,remCurrent=response.responseText.match(remPattern);if(current!==null&&current.length!==0){found=found.concat(current);foundProps=foundProps.concat(remCurrent)}if(i===sheets.og-1)buildIt()},buildIt=function(){var pattern=/[\w\d\s\-\/\\%#:,.'"*()]+\d*\.{0,1}\d+rem[\w\d\s\-\/\\%#:,.'"*()]*[;}]/g;
for(var i=0;i<found.length;i++){rules=rules+found[i].substr(0,found[i].indexOf("{")+1);var current=found[i].match(pattern);for(var j=0;j<current.length;j++){rules=rules+current[j];if(j===current.length-1&&rules[rules.length-1]!=="}")rules=rules+"\n}"}}parseIt()},parseIt=function(){for(var i=0;i<foundProps.length;i++)css[i]=Math.round(eval(foundProps[i].substr(0,foundProps[i].length-3)*fontSize))+"px";loadCSS()},loadCSS=function(){for(var i=0;i<css.length;i++)if(css[i])rules=rules.replace(foundProps[i],
css[i]);rules=removeComments(rules);var remcss=document.createElement("style");remcss.setAttribute("type","text/css");remcss.id="remReplace";document.getElementsByTagName("head")[0].appendChild(remcss);remcss.styleSheet.cssText=rules},xhr=function(url,callback,i){var xhr=getXMLHttpRequest();xhr.open("GET",url,true);xhr.send();if(window.XMLHttpRequest)xhr.onreadystatechange=function(){if(xhr.readyState===4)callback(xhr,i);else;};else xhr.onreadystatechange=new function(){if(xhr.readyState===4)callback(xhr,
i);else;}},removeComments=function(css){var start=css.search(/\/\*/),end=css.search(/\*\//);if(start>-1&&end>start){css=css.substring(0,start)+css.substring(end+2);return removeComments(css)}else return css},getXMLHttpRequest=function(){if(window.XMLHttpRequest)return new XMLHttpRequest;else return new ActiveXObject("Microsoft.XMLHTTP")};if(!cssremunit()){var rules="",sheets=[],found=[],foundProps=[],css=[],body=document.getElementsByTagName("body")[0],fontSize="";if(body.currentStyle)if(body.currentStyle["fontSize"].indexOf("px")>=
